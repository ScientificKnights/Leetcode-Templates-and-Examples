...
private:
    ...
    int dfs(int pos,int M, const vector<int>& sum){
        if(dp[pos].count(M)) return dp[pos][M];
        if(pos>=sum.size()) return 0;
        int res=INT_MIN,n=sum.size();
        for(int i=pos;i<min(pos+2*M,n);i++){
            int before=(pos==0)?0:sum[pos-1];
            res=max(res,sum[i]-before+sum.back()-sum[i]-dfs(i+1,max(M,i-pos+1),sum));
        }
        dp[pos][M]=res;
        return res;
    }
